...
Task(BUILD)
	...
	.Does(() => {
		...
		{
			/*
			DotNetCoreBuild(ROOT_PATH + libproject + "/" + libproject + ".csproj", new() {
				MSBuildSettings = new() {

				}
			});
			*/
			MSBuild(ROOT_PATH + libproject + "/" + libproject + ".csproj", c => {
...
var deleteSettings = new DeleteDirectorySettings() { Recursive = true };

/* Cleanup tasks */
Task(CLEAN)
	.Does(() => {
		DeleteDirectories(GetDirectories(ROOT_PATH + "**/bin"), deleteSettings);
		DeleteDirectories(GetDirectories(ROOT_PATH + "**/obj"), deleteSettings);
	});
...
Task(FULL_CLEAN)
	...
	.Does(() => {
		...
		{
			DeleteDirectory(DEPLOYMENT_DIRECTORY, deleteSettings);
		}

		if(DirectoryExists(ARTIFACTS_DIRECTORY))
		{
			DeleteDirectory(ARTIFACTS_DIRECTORY, deleteSettings);
		}
	});
